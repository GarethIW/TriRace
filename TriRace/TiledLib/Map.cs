			...
			for (int i = 0; i < numLayers; i++)
			{
				...
				else if (type == "objectgroup")
				{
					...
					for (int j = 0; j < numObjects; j++)
					{
                        ...
                        List<Point> objLinePoints = reader.ReadObject<List<Point>>();
                        List<Point> objPolyPoints = reader.ReadObject<List<Point>>();
						PropertyCollection objProps = new PropertyCollection();
						objProps.Read(reader);

						objects.Add(new MapObject(objName, objType, objLoc, objLinePoints, objPolyPoints, objProps));
					}
